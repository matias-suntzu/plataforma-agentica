"""Herramientas para métricas globales - FIXED v2"""
...
def get_all_campaigns_metrics_func(
    ...
    # ✅ FIX: Acceder solo a atributos que existen en el schema
    date_preset = input.date_preset if hasattr(input, 'date_preset') else 'last_7d'
    
    # Intentar obtener date_start/date_end si existen (opcional)
    date_start = getattr(input, 'date_start', None)
    date_end = getattr(input, 'date_end', None)
    
    # Intentar obtener metrics si existe
    metrics = getattr(input, 'metrics', ["spend", "clicks", "impressions", "conversions"])
    ...
    try:
        ...
        # ✅ Usar date_start/date_end si están definidos, sino usar date_preset
        ...
        totals = {
            ...
            'campaigns': [],
            'period': date_preset if not (date_start and date_end) else f"{date_start} to {date_end}"
        }
